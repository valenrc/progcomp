...
int s, a, b;
inline ulong v_in(ulong v){ return v; }
inline ulong v_out(ulong v) { return v + s * a; }
...
int main() {
  ...
  while(p--){
    ...
    vb starting_points(s * a, false);

    forn(_,b){
      int x, y; cin >> x >> y;
      x--; y--;
      starting_points[x * a + y] = true;
    }
    ...
    // v_in = v
    // v_out = v + s*a
    // v_in -> v_out con capacidad 1
    dinics::flowgraph<> G(s*a*2 + 2);
    dinics::flowgraph<>::Vertex source = s*a*2;
    dinics::flowgraph<>::Vertex sink   = s*a*2 + 1;

    // agregar aristas de v_in a v_out con capacidad 1
    // y tambien las de source
    forn(i,s*a){
      if(starting_points[i]){
        G.add_edge(source, v_in(i), 1);
        G.add_edge(i, i+s*a, 1);
      }else{
        G.add_edge(i,i+s*a, 1);
      }
    }

    // agregar aristas que representan el grid graph (capacidad 1)
    forn(i, s){
      forn(j,a){
        ulong v = v_out(i * a + j);
        
        // agrego las aristas del borde del mapa hacia sink
        if(i == s-1 or i == 0) {
          G.add_edge(v, sink, 1);
        }else{
          if(j == 0 or j == a-1) {
        ...
        if(i < s-1) G.add_edge(v, v_in((i+1) * a + j), 1);    // (i+1, j)
        if(j < a-1) G.add_edge(v, v_in(i * a + (j + 1)), 1);  // (i, j+1)
        if(i > 0)   G.add_edge(v, v_in((i-1) * a + j), 1);    // (i-1, j)
        if(j > 0)   G.add_edge(v, v_in(i * a + (j - 1)), 1);  // (i, j-1)
      }
    }

    // calcular el max flow
    ll maxflow = G.maxflow(source, sink);
    if(maxflow == b){
      cout << "possible\n";
    }else{
      cout << "not possible\n";
