...
ll nk, np; // cant. de categorias y cant. de problemas

void print_flowgraph(const dinics::flowgraph<>& G) {
  const auto& residual = G.get_residual();
  for (size_t from = 0; from < residual.size(); ++from) {
    cout << "Vertex " << from << ":\n";
    for (const auto& edge : residual[from]) {
      cout << "  -> " << edge.to 
...
int main() {
  ...
  while((cin >> nk >> np) && (nk || np)){
    // Los primeros nk nodos son la cantidad de categorias, los demas son los np problemas.
    // Ignoro el vertice 0
    flowgraph<> G(4+nk+np);
    flowgraph<>::Vertex source = nk+np+1;
    flowgraph<>::Vertex aux    = nk+np+2;
    flowgraph<>::Vertex sink   = nk+np+3;

    G.add_edge(source, aux, np);

    // cantidad de problemas de cada categoria i
    ll max_flow_res = 0;  // si el max_flow da esto, devolver 1 e imprimir las aristas saturadas
    forn(i, nk){
      ll q; cin >> q;
      max_flow_res+=q;
      G.add_edge(i+1, sink, q); 
    }

    // cantidad de categorias de cada problema i
    fordn(i, 1, np+1){
      ll prob = i+nk; // indice del i-esimo problema en el grafo
      int cant_categorias; cin >> cant_categorias;

      // aux -> prob con c=1
      G.add_edge(aux, prob, 1);
    
      forn(_, cant_categorias){
        ll cat; cin >> cat;
        G.add_edge(prob, cat, 1);
      }
    }

    // encontrar flujo max
    ll maxf = G.maxflow(source, sink);

    if(maxf == max_flow_res){
      cout << "1\n";
      // print G.saturated_edges(); es un vector<edge>
      vvi res(nk);
      for(auto& e : G.get_saturated_edges()){
        if((e.from >= nk+1 and e.from <= nk+np) and (e.to >= 1 and e.to <= nk)){
          // e.from es categoria, e.to es problema
          res[e.to-1].push_back(e.from - nk);
        }
      }
